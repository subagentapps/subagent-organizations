...
<p>The header part consists of header fields. Each header field is comprised of a name and a value, separated by ‘: ‘ (a colon and a space). The structure of header fields conforms to the <a href="https://tools.ietf.org/html/rfc7230#section-3.2">HTTP semantic</a>. Each header field is terminated by ‘\r\n’. Considering the last header field and the overall header itself are each terminated with ‘\r\n’, and that at least one header is mandatory, this means that two ‘\r\n’ sequences always immediately precede the content part of a message.</p>
...
<p>The protocol uses the following definitions for integers, unsigned integers, decimal numbers, objects and arrays:</p>
...
<div class="language-typescript highlighter-rouge"><div class="highlight"><pre class="highlight"><code><span class="kr">interface</span> <span class="nx">ResponseMessage</span> <span class="kd">extends</span> <span class="nx">Message</span> <span class="p">{</span>
	 ...
	 */</span>
	<span class="nl">result</span><span class="p">?:</span> <span class="nx">LSPAny</span><span class="p">;</span>
...
<p>An example would be a request sent from the client to the server to request a hover value for a symbol at a certain position in a text document. The request’s method would be <code class="language-plaintext highlighter-rouge">textDocument/hover</code> with a parameter like this:</p>
...
<p>The protocol currently assumes that one server serves one tool. There is currently no support in the protocol to share one server between different tools. Such sharing would require additional protocol e.g. to lock a document to support concurrent editing.</p>
...
<p>We also maintain a node module to parse a string into <code class="language-plaintext highlighter-rouge">scheme</code>, <code class="language-plaintext highlighter-rouge">authority</code>, <code class="language-plaintext highlighter-rouge">path</code>, <code class="language-plaintext highlighter-rouge">query</code>, and <code class="language-plaintext highlighter-rouge">fragment</code> URI components. The GitHub repository is <a href="https://github.com/Microsoft/vscode-uri">https://github.com/Microsoft/vscode-uri</a>, and the npm module is <a href="https://www.npmjs.com/package/vscode-uri">https://www.npmjs.com/package/vscode-uri</a>.</p>
...
<p>The token received via the <code class="language-plaintext highlighter-rouge">workDoneToken</code> property in a request’s param literal is only valid as long as the request has not send a response back. Canceling work done progress is done by simply
canceling the corresponding request.</p>
...
<p>Before requesting information from the server (e.g., <code class="language-plaintext highlighter-rouge">textDocument/completion</code> or <code class="language-plaintext highlighter-rouge">textDocument/signatureHelp</code>), the client must ensure that the document’s state is synchronized with the server to guarantee reliable results.</p>

<p>The following example shows how the client should synchronize the state when the user has continuous input, assuming user input triggered <code class="language-plaintext highlighter-rouge">textDocument/completion</code>:</p>
...
<table>
  ...
  <tbody>
    <tr>
      <td>5</td>
      <td>document change one</td>
      <td>sync document <code class="language-plaintext highlighter-rouge">v5</code> to the server</td>
      <td><code class="language-plaintext highlighter-rouge">textDocument/didChange</code></td>
    </tr>
    <tr>
      <td>5</td>
      <td>-</td>
      <td>request from the server, based on document <code class="language-plaintext highlighter-rouge">v5</code>
</td>
      <td><code class="language-plaintext highlighter-rouge">textDocument/completion</code></td>
    </tr>
    <tr>
      <td>6</td>
      <td>document change two</td>
      <td>sync document <code class="language-plaintext highlighter-rouge">v6</code> to the server</td>
      <td><code class="language-plaintext highlighter-rouge">textDocument/didChange</code></td>
    </tr>
  </tbody>
</table>
...
<span class="k">export</span> <span class="kd">type</span> <span class="nx">NotebookDocumentFilter</span> <span class="o">=</span> <span class="p">{</span>
	...
	<span class="cm">/** A Uri scheme, like `file` or `untitled`. */</span>
	<span class="nl">scheme</span><span class="p">?:</span> <span class="kr">string</span><span class="p">;</span>
...
<p>In both modes, notebook cell text documents are treated as regular text documents. They are always synchronized using incremental sync.</p>
...
<ul>
  ...
  <li>property type: <code class="language-plaintext highlighter-rouge">NotebookDocumentSyncOptions | NotebookDocumentSyncRegistrationOptions</code> where <code class="language-plaintext highlighter-rouge">NotebookDocumentOptions</code> is defined as follows:</li>
</ul>
...
<p><em>Registration Options</em>: <code class="language-plaintext highlighter-rouge">notebookDocumentSyncRegistrationOptions</code> defined as follows:</p>
...
<p>When the client sends a hover request, the position typically refers to the position immediately to the left of the character being hovered over. For example, when a user hovers over a character <code class="language-plaintext highlighter-rouge">c</code> at offset <code class="language-plaintext highlighter-rouge">n</code>, the client typically sends position <code class="language-plaintext highlighter-rouge">n</code> (the position before the character). However, how servers interpret this position and what hover information they return is language and implementation specific.</p>
...
<ul>
  <li>at index <code class="language-plaintext highlighter-rouge">5*i</code>   - <code class="language-plaintext highlighter-rouge">deltaLine</code>: token line number, relative to the start of the previous token</li>
  <li>at index <code class="language-plaintext highlighter-rouge">5*i+1</code> - <code class="language-plaintext highlighter-rouge">deltaStart</code>: token start character, relative to the start of the previous token (relative to 0 or the previous token’s start if they are on the same line)</li>
...
<p>A server is allowed to compute the semantic tokens for a broader range than requested by the client. However if the server does the semantic tokens for the broader range must be complete and correct. If a token at the beginning or end only partially overlaps with the requested range the server should include those tokens in the response.</p>
...
<p>Consider the client announcing the <code class="language-plaintext highlighter-rouge">label.location</code> property as a property that can be resolved lazily using the client capability</p>
...
<span class="k">export</span> <span class="kr">interface</span> <span class="nx">DiagnosticClientCapabilities</span> <span class="p">{</span>
	 ...
	 * Clients supporting tags have to handle unknown tags gracefully.
	 */</span>
	<span class="nl">tagSupport</span><span class="p">?:</span> <span class="nx">ClientDiagnosticsTagOptions</span><span class="p">;</span>
...
<p><em>Request</em>:</p>
<ul>
  <li>method: <code class="language-plaintext highlighter-rouge">textDocument/diagnostic</code>.</li>
...
<p><em>Request</em>:</p>
<ul>
  <li>method: <code class="language-plaintext highlighter-rouge">workspace/diagnostic</code>.</li>
...
<p>Consider the client announcing the <code class="language-plaintext highlighter-rouge">edit</code> property as a property that can be resolved lazily using the client capability</p>
...
<span class="kr">interface</span> <span class="nx">FileEvent</span> <span class="p">{</span>
	 ...
	 * The change type.
	 */</span>
	<span class="nl">type</span><span class="p">:</span> <span class="nx">FileChangeType</span><span class="p">;</span>
<span class="p">}</span>
</code></pre></div></div>
